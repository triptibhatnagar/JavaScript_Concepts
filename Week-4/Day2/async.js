...
Single-theraded - code is executed on one thread only.
...
-------------
HOW IT IS DONE ?
-> JS engine (alone js execution engine) is made up of callstack(global execution context is formed and functions are loaded one by one when a program is executed) and heap memory(memory allocations are done). First, a call of asynchronous code is made and transferred to Web API / Node API.
